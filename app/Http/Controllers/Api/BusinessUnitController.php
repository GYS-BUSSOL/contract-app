...
class BusinessUnitController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'status' => 200,
                'message' => 'Successfully retrieved area business unit data',
                'data' => [
                    'rows' => $dataGet,
            ...
            return response()->json([
                'status' => 500,
                'message' => 'Failed to retrieve area business unit data',
                'data' => [
                    'rows' => [],
                    'total_record' => 0
                ],
    ...
    public function add(Request $request)
    {
        ...
        try {
            $validated['group_dimension'] = "Business Unit";
            $validated['is_active'] = 0;
            $this->mbcw->create($validated);

            return response()->json([
                'status' => 200,
                'message' => 'Area business unit created successfully'
            ], 200);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 500,
                'error' => 'Server error',
                'message' => 'Area business unit failed to create',
            ], 500);
        }
    }
    ...
    {
        try {
            $areaBU = $this->mbcw->firstWhere('id', $id);
            if (empty($areaBU)) {
                return response()->json([
                    "status" => 404,
                    "message" => "Area business unit not found",
                ], 404);
            }
            return response()->json([
                "status" => 200,
    ...
    public function update(Request $request, int $id)
    {
        ...
        try {
            ...
            if ($mbcw->update($validated)) {
                return response()->json([
                    "status" => 200,
                    "message" => "Successfully updated area business unit data",
                ], 200);
            }
        } catch (\Exception $e) {
            return response()->json([
                "status" => 500,
                "message" => "Failed to updated area business unit data",
                "error" => "Server error",
            ], 500);
        }
    }
    ...
    public function destroy(int $id)
    {
        try {
            ...
            if ($mbcw->delete()) {
                return response()->json([
                    "status" => 200,
                    "message" => "Successfully deleted area business unit data"
                ], 200);
            }
        } catch (\Exception $e) {
            return response()->json([
                "status" => 500,
                "message" => "Failed to delete area business unit data",
                "error" => "Server error",
            ], 500);
        }
    }
