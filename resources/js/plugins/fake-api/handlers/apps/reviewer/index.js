...
export const handlerAppsReviewer = [
    ...
    // Get Single Reviewer Detail
    http.get(('/api/apps/reviewer/:id'), async ({ params }) => {
      const reviewerId = Number(params.id)
      const reviewer = db.reviewers.find(e => e.id === reviewerId)
      if (!reviewer) {
        return HttpResponse.json({ message: 'Reviewer not found' }, { status: 404 })
      }
      else {
        return HttpResponse.json({
          ...reviewer,
    ...
    // Delete Reviewer
    http.delete(('/api/apps/reviewer/:id'), async ({ params }) => {
      const reviewerId = Number(params.id)
      const reviewerIndex = db.reviewers.findIndex(e => e.id === reviewerId)
      if (reviewerIndex === -1) {
        return HttpResponse.json('Reviewer not found', { status: 404 })
      }
      else {
        db.reviewers.splice(reviewerIndex, 1)
    ...
    // 👉 Add reviewer
    http.post(('/api/apps/reviewer'), async ({ request }) => {
      const reviewer = await request.json()

      db.reviewers.push({
        ...reviewer,
        id: db.reviewers.length + 1,
      })
      
      return HttpResponse.json({ body: reviewer }, { status: 201 })
    }),
]
