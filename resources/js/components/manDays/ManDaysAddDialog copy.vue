...
const manDaysData = ref({})

const fetchMandaysEdit = async () => {
    try {
    isLoading.value = true;
    const { data } = await useApi(`/configurations/man-days-rate/edit/${mandaysId.value}`);
    
    if (data.value) {
      isLoading.value = false;
      manDaysData.value = data.value;
    } else {
      throw new Error("Data value is not true");
    }
    ...
    emit('update:isDialogVisible', false);
    console.error('Error fetching man days:', error);
  }

};

watch([() => mandaysId.value, () => typeDialog.value], () => {
  if (typeDialog.value === 'Edit' && mandaysId.value) {
    fetchMandaysEdit();
  }
});
...
<template>
  ...
  <VSnackbar
    ...
    >
      {{ isSnackbarResponseAlertColor == 'error' ? 'Internal server error' : 'Man Days created successfully' }}
      <template #actions>
        <VBtn
          color="white"
