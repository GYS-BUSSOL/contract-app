...
const jobTypeId = computed(() => props.jobTypeId)
const loadingBtn = ref([])
const jobTypeData = reactive({
  job_type: null,
});

const dialogModelValueUpdate = () => {
  jobTypeData.job_type = null;
  // Reset form
...
const fetchJobTypeEdit = async () => {
  try {
    ...
    const response = await $api(`/configurations/job-type/edit/${jobTypeId.value}`, {
      method: 'GET',
    ...
    if (dataResponse.status == 200) {
      ...
      jobTypeData.job_type = dataResult.job_type;
    } else {
      emit('update:isDialogVisible', false)
...
watch(
  [() => jobTypeId.value, () => typeDialog.value, () => props.fetchTrigger],
    ([newId,newType]) => {
  ...
  refVForm.value?.validate().then(({ valid }) => {
    try {
      if (valid) {
        ...
        emit("JobTypeData", { mode, formData: { ...jobTypeData }, dialogUpdate: dialogModelValueUpdate });
      } else {
        loadingBtn.value[0] = false
      }
    } catch (err) {
