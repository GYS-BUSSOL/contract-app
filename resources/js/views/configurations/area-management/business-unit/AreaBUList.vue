...
const {
  data: areaBUData,
  execute: fetchAreaBU,
} = await useApi(createUrl('/configurations/area-management-bu/search', {
  query: {
    q: searchQuery,
...
const areaBU = computed(() => areaBUData.value.areaBU)
const totalAreaBU = computed(() => areaBUData.value.totalAreaBU)
const totalActiveCount = computed(() => areaBUData.value.totalActiveCount)
const totalNotActiveCount = computed(() => areaBUData.value.totalNotActiveCount)
...
const resolveAreaBUStatusVariant = stat => {
  if (stat == 0 && stat != null )
    return 'success'
  else if(stat == 1)
    return 'error'
...
const fetchAddData = async (areaBUData, clearedForm) => {
  try {
    ...
    if(responseParse?.status == 200) {
      ...
      isAreaBUDialogVisible.value = false
    } else {
      alertErrorResponse()
      throw new Error("Created data failed");
    }
...
const deleteAreaBU = async id => {
  try {
    ...
    if(responseParse?.status == 200) {
      ...
      const responseMessage = responseParse?.message;
      successMessages.value = responseMessage;
      isAreaBUDialogDeleteVisible.value = false;
    } else {
      alertErrorResponse()
      throw new Error("Deleted data failed");
    }
    ...
    <VCard class="mb-6">
      ...
      >
        ...
        <template #item.is_active="{ item }">
          <VChip
            :color="resolveAreaBUStatusVariant(item.is_active)"
            size="small"
            label
